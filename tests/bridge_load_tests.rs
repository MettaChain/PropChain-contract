...
mod bridge_extreme_load_tests {
    ...
    impl MockBridge {
        ...
        fn rejected_count(&self) -> u64 {
            self.rejected_counter.load(Ordering::SeqCst)
        }
    ...
    fn test_high_volume_concurrent_request_creation() {
        ...
        println!(
            "High-volume creation: {} requests created concurrently",
            USERS
        );
    }
    ...
    fn test_rate_limit_enforced_under_burst() {
        ...
        for i in 0..BURST {
            if bridge.initiate(0, 1, i, None).is_some() {
                accepted += 1;
            } else {
                rejected += 1;
            }
        ...
        println!(
            "Rate limit burst: accepted={}, rejected={}",
            accepted, rejected
        );
    }
    ...
    fn test_duplicate_signatures_rejected() {
        ...
        assert_eq!(
            ...
            1,
            "only one sig from same operator"
        );
    ...
    fn test_full_pipeline_throughput_500_requests() {
        ...
        let rids: Vec<u64> = (0..N)
            .map(|i| bridge.initiate(i, 1, 0, None).unwrap())
            .collect();
        for &rid in &rids {
            bridge.sign(rid, 0, 0);
        }
        ...
        assert!(
            elapsed.as_secs() < 5,
            "pipeline took {:?}, expected < 5s",
            elapsed
        );
        println!(
            "Pipeline: {} requests in {:?} ({:.0} req/s)",
            N,
            elapsed,
    ...
    fn test_emergency_pause_rejects_requests() {
        ...
        assert_eq!(
            ...
            before,
            "no new requests while paused"
        );
        assert!(
            results.iter().all(|r| r.is_none()),
            "all post-pause requests rejected"
        );
    ...
    fn test_expired_requests_rejected_under_load() {
        ...
        let bridge = MockBridge::new(N, 1000);
        let rids: Vec<u64> = (0..N)
            .map(|i| bridge.initiate(i, 1, 0, Some(1000)).unwrap())
            .collect();
    ...
    fn test_concurrent_recovery_after_expiry() {
        ...
        let bridge = MockBridge::new(N, 1000);
        let rids: Vec<u64> = (0..N)
            .map(|i| bridge.initiate(i, 2, 0, Some(500)).unwrap())
            .collect();
        for &rid in &rids {
            bridge.sign(rid, 0, 1000);
        } // trigger expiry
        assert_eq!(bridge.count_by_status(BridgeStatus::Expired), N);
    ...
    fn test_state_consistency_under_mixed_load() {
        ...
        println!(
            "Mixed load: total={}, executed={}",
            bridge.total_requests(),
            bridge.executed_count()
        );
    }
    ...
    fn test_multi_chain_load_distribution() {
        ...
        for (idx, &count) in c.iter().enumerate() {
            assert_eq!(
                count, PER_CHAIN as u64,
                "chain {} got {} requests, expected {}",
                idx, count, PER_CHAIN
            );
        }
    ...
    fn test_sustained_load_no_throughput_degradation() {
        ...
        assert!(
            ratio < 10.0,
            "throughput degraded {:.1}x between batch 1 and {}",
            ratio,
            BATCHES
        );
        assert_eq!(bridge.total_requests(), (BATCHES * BATCH_SIZE) as u64);
        println!(
            "Sustained: {} batches, degradation ratio {:.2}x",
            BATCHES, ratio
        );
    }
}
