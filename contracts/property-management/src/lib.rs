...
mod property_management {
    ...
    impl PropertyManagement {
        ...
        /// Return up to `limit` leases belonging to `owner` (as landlord), starting at
        /// sequential offset `offset`.  Limits are capped at [`MAX_PAGE`].
        ///
        /// Legacy callers should continue using [`get_lease`] for individual look-ups.
        #[ink(message)]
        pub fn get_leases_by_owner(
            &self,
            owner: AccountId,
            offset: u64,
            limit: u64,
        ) -> Vec<Lease> {
            let limit = limit.min(Self::MAX_PAGE);
            let mut result = Vec::new();
            let mut skipped: u64 = 0;
            for id in 1..=self.lease_counter {
                if let Some(lease) = self.leases.get(id) {
                    if lease.landlord == owner {
                        if skipped < offset {
        ...
        /// Total number of leases owned by `owner` (landlord).  Use this together with
        /// [`get_leases_by_owner`] to drive pagination on the client side.
        #[ink(message)]
        pub fn count_leases_by_owner(&self, owner: AccountId) -> u64 {
            let mut count: u64 = 0;
            for id in 1..=self.lease_counter {
                if let Some(lease) = self.leases.get(id) {
                    if lease.landlord == owner {
        ...
        /// Return up to `limit` disputes, starting at sequential offset `offset`.
        /// Limits are capped at [`MAX_PAGE`].
        ///
        /// Legacy callers should continue using [`get_dispute`] for individual look-ups.
        #[ink(message)]
        pub fn get_disputes_paginated(&self, offset: u64, limit: u64) -> Vec<DisputeCase> {
            let limit = limit.min(Self::MAX_PAGE);
            let mut result = Vec::new();
            let start = offset.saturating_add(1);
            let end = start.saturating_add(limit);
            for id in start..end {
                if id > self.dispute_counter {
                    break;
                }
        ...
        /// Total number of recorded disputes.
        #[ink(message)]
        pub fn count_disputes(&self) -> u64 {
            self.dispute_counter
        }

        /// Return up to `limit` expenses, starting at sequential offset `offset`.
        /// Limits are capped at [`MAX_PAGE`].
        ///
        /// Legacy callers should continue using [`get_expense`] for individual look-ups.
        #[ink(message)]
        pub fn get_expenses_paginated(&self, offset: u64, limit: u64) -> Vec<Expense> {
            let limit = limit.min(Self::MAX_PAGE);
            let mut result = Vec::new();
            let start = offset.saturating_add(1);
            let end = start.saturating_add(limit);
            for id in start..end {
                if id > self.expense_counter {
                    break;
                }
                if let Some(expense) = self.expenses.get(id) {
                    result.push(expense);
                }
            }
            result
        ...
        #[ink(message)]
        pub fn count_expenses(&self) -> u64 {
            self.expense_counter
        }

        /// Return up to `limit` maintenance requests, starting at sequential offset `offset`.
        /// Limits are capped at [`MAX_PAGE`].
        ///
        /// Legacy callers should continue using [`get_maintenance`] for individual look-ups.
        #[ink(message)]
        pub fn get_maintenance_paginated(
            &self,
            offset: u64,
            limit: u64,
            ...
            let end = start.saturating_add(limit);
            for id in start..end {
                if id > self.maintenance_counter {
                    break;
                }
                if let Some(req) = self.maintenance.get(id) {
                    result.push(req);
                }
            }
            result
        ...
        #[ink(message)]
        pub fn count_maintenance(&self) -> u64 {
